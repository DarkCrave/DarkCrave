...
void Shutdown()
{
    ...
    RenameThread("DarkCrave-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: DarkCrave.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: DarkCraved.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeDarkCraveamount=<n> " + _("Keep N DarkCrave anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DarkCrave is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DarkCrave is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DarkCrave version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "DarkCrave server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DarkCrave") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DarkCrave to complete") << "\n";
    ...
    nAnonymizeDarkCraveAmount = GetArg("-anonymizeDarkCraveamount", 0);
    if(nAnonymizeDarkCraveAmount > 999999) nAnonymizeDarkCraveAmount = 999999;
    if(nAnonymizeDarkCraveAmount < 2) nAnonymizeDarkCraveAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeDarkCraveAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize DarkCrave Amount %d\n", nAnonymizeDarkCraveAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1DCC+1000 == (.1DCC+100)*10
       10DCC+10000 == (1DCC+1000)*10
    */
