...
int nAnonymizeDarkCraveAmount = 500;
...
#else
    const char* pszModule = "DarkCrave";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DarkCrave
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DarkCrave
    // Mac: ~/Library/Application Support/DarkCrave
    // Unix: ~/.DarkCrave
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DarkCrave";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "DarkCrave.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "DarkCraved.pid"));
