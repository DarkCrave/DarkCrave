...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("DarkCrave"))
        return false;
...
{
    // Convert DarkCrave:// to DarkCrave:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("DarkCrave://"))
    {
        uri.replace(0, 12, "DarkCrave:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DarkCrave.lnk";
}
...
{
    return GetAutostartDir() / "DarkCrave.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DarkCrave-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DarkCrave-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("DarkCrave-Qt"));
